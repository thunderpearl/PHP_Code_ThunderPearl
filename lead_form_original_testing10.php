...
<div id="form" >
    ...
    <form method = "POST" action ="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"> 
      ...
      <select name="proptype" style="height:30px; border-radius:4px; background-color:#ccccff; color:#5c5cd6;font-family:Times; font-size:20px" >
        ...
        <input class = "line" type="text" name="state" id="state" placeholder="State" value = "<?php if(isset($_POST['state'])){ echo htmlentities($_POST['state']); } ?>" required="required" style="text-align:left"/>
        
        
        <label for="City" class="names"> City:</label>
        <input class = "line" type="text" name="City" id="City" placeholder="City" value = "<?php if(isset($_POST['City'])){ echo htmlentities($_POST['City']); } ?>" required="required" style="text-align:left"/>
        
        
        <label for="Budget" class="names"> Budget:</label>
        <input class = "line" type="text" name="budget" id="budget" placeholder="Budget" value = "<?php if(isset($_POST['budget'])){ echo htmlentities($_POST['budget']); } ?>" required="required" style="text-align:left"/>
        <br>
        ...
        <input class = "line" type="text" name="name" id="name" placeholder=" Name" value = "<?php if(isset($_POST['name'])){ echo htmlentities($_POST['name']); } ?>" required="required" style="text-align:left"/>
        
        <!--<br><br>-->
        <label style= "margin-left:20px" for="email" class="names">Email:</label>
        <input class="line" type="email" name="email" id="email" placeholder="Enter Email" value = "<?php if(isset($_POST['email'])){ echo htmlentities($_POST['email']); } ?>" required="required" style="text-align:left"/>
        
        <br><br><br>
        <label for="country" class="names"> Country:</label>
        <input class = "line" type="text" name="country" id="country" placeholder="Country Name" value = "<?php if(isset($_POST['country'])){ echo htmlentities($_POST['country']); } ?>" required="required" style="text-align:left"/>
        
        <br><br><br>
        <label for="landline" class="names"> Landline No.:</label>
        <input class = "line" type="tel" name="landline" id="landline" placeholder="Landline No." value = "<?php if(isset($_POST['landline'])){ echo htmlentities($_POST['landline']); } ?>" required="required" style="text-align:left"/>
        

        <label style= "margin-left:20px" for="mobile" class="names"> Mobile No.:</label>
        <input class = "line" type="tel" name="mobile" id="mobile" placeholder="Mobile" value = "<?php if(isset($_POST['mobile'])){ echo htmlentities($_POST['mobile']); } ?>" required="required" style="text-align:left"/>
        
        <br><br><br>
        <label for="address" class="names"> Address:</label>
        <input class = "line" type="text" name="address" id="address" placeholder="Address" value = "<?php if(isset($_POST['address'])){ echo htmlentities($_POST['address']); } ?>" required="required" style="text-align:left; width:30%"/>
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
        error_reporting(E_ALL);
            ...
            $sql="INSERT INTO kuldeep_singh_leadform_table(Name, Email, Address, Country, Landline, Mobile,Property_Req_ON, Type_of_Property, Prefered_State, Prefered_City, Budget_in_INR) 
