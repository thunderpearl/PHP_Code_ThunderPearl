...
<!DOCTYPE html>
<!-- This is the page for information fetching used by admin/sales representative to get the information of client/user from DB_Tables -->
<!-- For the code description please refer the file "kuldeep_registration_page_improved.php" -->
...
// This is for the "kuldeep_singh_property_table". 
 if(isset($_POST['fetch_data'])){
   $servername = "localhost";
    $username = "root";
    $password = "secret";
    $dbname = "kuldeep_singh_db_improved";
  ...
  $id=$_POST['id'];
   $sqluname_retire1 = "SELECT * from kuldeep_singh_property_table where id = '$id'";
   $sel_qry1=$db->query($sqluname_retire1);
   $fetch_data1=$sel_qry1->fetch_array(MYSQLI_ASSOC);  // Get the data in associative array "$fetch_array" and store it in "$fetch_data1".
   
   }
   
   
   // This is for the "kuldeep_singh_customer_table". 
    if(isset($_POST['fetch_data'])){
   $servername = "localhost";
    $username = "root";
    $password = "secret";
  ...
  if ($db->connect_errno) {
    ...
    exit();
		...
		$sqluname_retire2 = "SELECT * from kuldeep_singh_customer_table where Email = '$email'";
		$sel_qry2=$db->query($sqluname_retire2);
		$fetch_data2=$sel_qry2->fetch_array(MYSQLI_ASSOC);  // Get the data in associative array "$fetch_array" and store it in "$fetch_data2".
	}
?>
    ...
    <form method = "POST" action ="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
     ...
     <br><br>
		<!--This link will forward us to other page for setting the status and stage for the property form-->	
         <a href="http:\\localhost:8080\\sales_representative_page2_improved.php"><div><button class="proceed" type="button">Proceed</button></div></a>
