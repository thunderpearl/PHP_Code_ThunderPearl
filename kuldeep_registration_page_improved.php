...
<div id="form" >
    ...
    <form method = "POST" action ="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"> 
      ...
      <select name="proptype" style="height:30px; border-radius:4px; background-color:#ccccff; color:#5c5cd6;font-family:Times; font-size:20px" >
		...
		<!-- Here user will enter his/her personal information -->
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
        error_reporting(E_ALL);
                ...
                $dbname = "kuldeep_singh_db_improved";    // name of the database 
                ...
                // this is the condition to check the connection is established or not.
                if (!$conn1) {
                ...
                function test_input($data) 
                {
                    ...
                    return $data;
                       ...
                       // $_POST['uname'] will collect the data from field by reference.    
                        ...
                        // All the below code like above line will collect the data entered by user/client using the reference name.  
			...
			// This is the sql query which will insert the data collected from the fields into the "kuldeep_singh_property_table".
            $sql1 = "INSERT INTO kuldeep_singh_property_table(Property_Req_On, Type_Of_Property, Prefered_State, Prefered_City, Budget_In_INR, Email) 
					VALUES ('$sare', '$proptype', '$state', '$city', $budget, '$email')";
                      
			
			//  This will select a whole row with matching email.
			$sql2 = "SELECT * FROM kuldeep_singh_customer_table WHERE Email = '$email'";
			
			
			
			// This will insert the data colelcted from the field into the "kuldeep_singh_customer_table".
			$sql3 = "INSERT INTO kuldeep_singh_customer_table(Email, Name, Address, Country, Landline, Mobile) 
					VALUES ('$email', '$uname', '$add', '$country', $landn, $mob)";
			
			$ress = mysqli_query($conn1, $sql2);
                  
				  // Here it will check if there exist a row with entered email or not.   
                if (mysqli_num_rows($ress) == 0) 
                    {	
						...
						{ 
							mysqli_query($conn1, $sql1);
							echo "Old customer, Entry Created";
						}
			...
			{
				echo "This is in Class A Customer";
				
			}
			
			elseif($budget >= 10000000 && $budget <= 200000000)
			{
				echo "This is in Class B Customer";
			}
			
			elseif($budget >= 200000000)
			{
				echo "This is in Class C Customer";
			}
